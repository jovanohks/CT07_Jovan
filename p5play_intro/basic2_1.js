...
function updatenoun(){
    ...
    storytext = templates.replace("{noun}",nouninput.value()).replace("{verb}",verbinput.value()).replace("{adjetive}",adjinput.value()).replace("{adv}",adverb.value()).replace("{noun}",place.value())                    
    background("220");
    textSize(12);
